...
SELECT * FROM personnes 
WHERE metier Like '%i%i%' 
OR lower(metier) LIKE '%e%e%' 
OR metier LIKE '%a%a%' 
OR metier LIKE '%o%o%' 
OR metier LIKE '%u%u%' 
ORDER BY prenom;
...
VALUES
(1,'Fiat','Punto',1200.50, 'rose' , '2019-01-01'),
(2,'Lexus','LC',125750.00, 'vert' , '2020-01-27'),
(3,'Tesla','serie 7',104990.99, 'rouge','2021-01-01'),
(4,'Renault','hybride',75000.00, 'bleu' , '2022-01-01');

Alter table personnes add column id_voiture int;

select * from personnes;

UPDATE personnes set id_ma_voiture = 1 where id = 2;
UPDATE personnes set id_ma_voiture = 2 where id = 4;
UPDATE personnes set id_ma_voiture = 3 where id = 3;
UPDATE personnes set id_ma_voiture = 4 where id = 5;
